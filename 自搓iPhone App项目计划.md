...
### 六、WebDAV 同步 Bug 修复（2026-06-07 下午）

#### 问题：列出云端备份为空

**现象**：坚果云 WebDAV 配置完成后，上传备份成功（返回"✅ 上传成功"），但点击「刷新云端备份列表」始终显示"暂无云端备份文件"。

**根因**：`xml ^6.5.0` 包的 `findElements(name)` 方法在处理带命名空间的 XML 元素（如 `<d:response>`、`<d:propstat>`）时匹配异常。WebDAV PROPFIND 响应的全部元素均带有 `d:` 命名空间前缀。

**排查过程**：
| 尝试 | 结果 |
|------|------|
| `doc.findAllElements('response')` | 返回 0 个 |
| `doc.rootElement.findAllElements('response')` | 返回 0 个 |
| 手动递归遍历 `children` 按 `name.local` 比较 | ✅ 找到正确元素 |

**修复**：移除所有 `findElements` / `findAllElements` 调用，改为手动遍历 `children` + `name.local` 比较的辅助方法 `_findChildByLocalName()`。

#### 问题：时间显示为 GMT

**现象**：修复列表显示后，备份文件时间显示为 11:xx 而非 19:xx。

**根因**：`_parseHttpDate()` 返回 `DateTime.utc(...)`，未转换到本地时区（UTC+8）。

**修复**：`DateTime.utc(...)` → `DateTime.utc(...).toLocal()`。

#### 附：时区转换修复
- `_parsePropfind` 改用 `href` 提取文件名（`displayname` 在某些 PROPFIND 响应中可能缺失，`href` 必定存在）
- 清理所有 `debugPrint` 调试代码，production 代码干净

#### 编译验证
- `flutter analyze` — 0 errors ✅
- `flutter build windows --release` — 通过 ✅
- iOS 兼容性 — sync_service 使用纯 Dart（`http` + `xml`），无平台特定代码 ✅

### 七、GitHub 仓库迁移
- **原因**：旧账号 `mbecth` 账单支付失败，Actions 无法构建
- **新账号**：`maodouibaba`（https://github.com/maodouibaba/life_log_app）
- **SSH key**：从旧账号迁移到新账号（ed25519，443端口）
- **待办**：新账号 GitHub Actions 免费分钟数可用后重新触发构建
...
# 查看 Actions 编译结果
# https://github.com/maodouibaba/life_log_app/actions
```
...
D:\life_log_app/
├── lib/
│   ├── main.dart                        # 入口（含暖棕/金色主题、经典青绿主题两套）
│   ├── models/
│   │   ├── tag.dart                     # 树状标签
│   │   ├── entry.dart                   # 记录
│   │   ├── entry_tag.dart               # 记录-标签关联
│   │   ├── project.dart                 # 项目
│   │   ├── project_group.dart           # 项目分组
│   │   ├── attribute_tag.dart           # 属性标签
│   │   ├── attribute_tag_group.dart     # 属性标签分组
│   │   ├── space.dart                   # 入口
│   │   ├── entry_template.dart          # 模板
│   │   ├── checkin_item.dart            # 打卡事项
│   │   └── checkin_record.dart          # 打卡记录
│   ├── database/
│   │   ├── app_database.dart            # sqflite（含 exportToJson / importFromJson / mergeFromJson）
│   │   └── web_database.dart            # Web 内存数据库
│   ├── pages/
│   │   ├── home_page.dart               # 首页时间线
│   │   ├── entry_editor_page.dart       # 新建/编辑记录（磨砂弹层 _FrostedSheet）
│   │   ├── entry_detail_page.dart       # 只读详览页
│   │   ├── tag_manager_page.dart        # 树状标签管理
│   │   ├── attribute_tag_manager_page.dart   # 属性标签管理
│   │   ├── project_manager_page.dart    # 项目管理
│   │   ├── list_view_page.dart          # 列表/筛选/批量操作
│   │   ├── data_migration_page.dart     # JSON 导入导出
│   │   ├── sync_settings_page.dart      # 坚果云同步
│   │   ├── lock_screen.dart             # 隐私锁
│   │   ├── space_selector_page.dart     # 入口选择
│   │   ├── checkin_page.dart            # 打卡页面
│   │   ├── checkin_item_manager_page.dart   # 打卡事项管理
│   │   ├── template_manager_page.dart   # 模板管理
│   │   └── template_editor_page.dart    # 模板编辑器
│   ├── services/
│   │   ├── export_service.dart          # Excel 导出
│   │   ├── sync_service.dart            # 坚果云 WebDAV 同步
│   │   ├── ai_service.dart              # AI 助写
│   │   ├── undo_manager.dart            # 撤销管理器
│   │   ├── theme_settings.dart          # 主题风格 + 明暗模式
│   │   ├── privacy_settings.dart        # 隐私锁
│   │   └── seed_data.dart               # 演示种子数据
│   ├── widgets/
│   │   ├── undo_button.dart             # 撤销按钮
│   │   └── undo_banner.dart             # 撤销横幅
│   └── utils/
│       └── text_formatter.dart          # 文本格式渲染
├── ios/                                 # iOS 原生配置
├── windows/                             # Windows 运行支持
├── web/                                 # Web 支持
├── .github/workflows/
│   └── build-ios.yml                    # GitHub Actions 自动编译
└── CLAUDE.md                            # 项目规则（禁止未授权推送）
```
...
### 一、UI 主题全面改造 — 暖棕/金色磨砂风格

**改造成果**：新增两套可切换的主题风格，全局 UI 大升级。

| 风格 | 说明 |
|------|------|
| 🎨 **暖棕/金色**（默认） | 深暖棕黑背景 + 金色强调色，圆角卡片 + 半透明磨砂质感 |
| 🫒 **经典青绿** | 原 Material 3 teal 风格，保留老用户习惯 |

**全局主题修改项（16 项）**：
- `ColorScheme` 自定义暖棕/金色替代 `fromSeed(Colors.teal)`
- `AppBarTheme` — 扁平半透明、iOS 风格
- `CardTheme` — **18px** 大圆角、半透明底色、暖色细边框
- `DialogTheme` — **22px** 圆角、磨砂半透背景
- `BottomSheetTheme` — **24px** 顶圆角、模态半透明遮罩
- `InputDecorationTheme` — **14px** 圆角、filled 磨砂
- `FilledButtonTheme` / `OutlinedButtonTheme` / `TextButtonTheme` — 金色主色、**14px** 圆角
- `FloatingActionButtonTheme` — **18px** 圆角、无阴影
- `ListTileTheme`、`ChipTheme`、`SnackBarTheme`、`DividerTheme`
- `PopupMenuTheme`、`SwitchTheme`、`CheckboxTheme`、`RadioTheme`

**磨砂玻璃效果**：
- `_FrostedSheet` widget — `BackdropFilter` + `ImageFilter.blur(sigmaX: 24)` 实现真实毛玻璃
- 卡片/弹窗使用半透明底色（alpha 0.85~0.92）模拟磨砂层次感

**前端硬编码颜色清理（跳过约 50 处）**：
- 全部 `Colors.grey` → `theme.colorScheme.onSurfaceVariant`
- 全部 `Colors.red` → `theme.colorScheme.error`
- 全部 `Colors.teal` → gold / primary
- 全部 `Colors.orange` → `tertiary` / `tertiaryContainer`
- 全部 `Colors.green` → `primary`

**涉及文件**：`main.dart`、`home_page.dart`、`space_selector_page.dart`、`list_view_page.dart`、`entry_editor_page.dart`、`sync_settings_page.dart`、`data_migration_page.dart`、`template_manager_page.dart`、`tag_manager_page.dart`、`project_manager_page.dart`、`undo_banner.dart`、`undo_button.dart`

### 二、主题风格切换机制

**新增** `ThemeSettings.style` 字段（`'warm'` / `'classic'`），持久化到数据库 `settings` 表。

- 首页 `⋯` → 选择主题 → 弹窗上半部分选择"主题风格"（暖棕/金色 ↔ 经典青绿），下半部分选择"明暗模式"
- `main.dart` 根据 `ThemeSettings().style` 自动选择对应的亮/暗主题构建

### 三、备份/同步补全模板和打卡数据

**问题**：`exportToJson()` / `importFromJson()` / `mergeFromJson()` 三处均缺少 `templates` 表和 `checkin_items` / `checkin_records` 表的处理。

**修复**：
1. `exportToJson()` — 增加 `templates`、`template_tags`、`template_attribute_tags`、`checkin_items`、`checkin_records` 五张表的导出
2. `importFromJson()` — 增加对应表的清理 + 导入
3. `mergeFromJson()` — 增加对应表的合并逻辑（`ConflictAlgorithm.ignore`）

### 四、锁屏 Bug 修复

**问题 1**：Windows 开启生物识别（面容/指纹）后锁死

**根因**：Windows 上 `local_auth` 插件的 `canUseBiometric()` 返回 `true`（Windows Hello），但 `authenticateBiometric()` 会卡死或失败，且无密码回退。

**修复**：
- `PrivacySettings.isBiometricSupported` — Windows/Linux 返回 `false`，自动关闭生物识别
- `authenticateBiometric()` 加 **15 秒 timeout** 防止卡死
- 锁屏底部增加**「直接进入」保底按钮**，无论如何都能解锁
- 无生物识别 + 无密码时 `_checkLock()` 自动解锁

**问题 2**：最小化→最大化→输密码后，画面跳到入口一而非当前入口

**根因**：`_AppEntryState._spaceId` 保存在内存中，锁屏隐藏 Widget 后状态丢失，`_init()` 默认取 `spaces.first.id`。

**修复**：
- `_onSpaceSelected()` 将 `spaceId` 写入数据库 `settings` 表（key: `last_space_id`）
- `_init()` 启动时优先从 DB 恢复上次使用的入口 ID

### 五、打卡按钮随窗口放大问题

**问题**：Windows 最大化时，打卡页面的 GridView 按钮（3 列）无限拉伸，变得巨大。

**修复**：
- 打卡页面内容整体居中 + `ConstrainedBox(maxWidth: 520)` 
- GridView 额外 `ConstrainedBox(maxWidth: 480)` 确保按钮宽高比合理
- 最大化窗口时内容居中不再拉伸

### 六、Git 仓库规则

**新增** `CLAUDE.md`：禁止未经明确指令推送代码（`git push` 需用户先说"推"/"推送"/"push"）。
...
- [x] ~~**模板/打卡数据未备份**~~ — 已修复
- [x] ~~**Windows 生物识别锁死**~~ — 已修复
- [x] ~~**锁屏后入口丢失**~~ — 已修复

### 近期（测试验证）
- [ ] 用最新 commit 生成 .ipa 安装测试全部功能
- [ ] 验证模板和打卡数据在备份/同步中的完整性

### 功能开发
- [x] ~~WebDAV 同步对接（`sync_service.dart` 需实现）~~ — 已实现
- [x] ~~模板创建与导入功能~~ — 已实现（6月14日）
- [x] ~~打卡功能（含统计分析）~~ — 已实现（6月14日）
- [x] ~~UI 美化（暖棕/金色磨砂主题）~~ — 已实现（6月14日）
- [x] ~~模板/打卡加入备份同步~~ — 已实现
- [ ] 更多写作风格
...
- **国内镜像**：FLUTTER_STORAGE_BASE_URL / PUB_HOSTED_URL
- **GitHub 仓库**：https://github.com/maodouibaba/life_log_app（私有），SSH 走 443 端口
